...
Components.utils.import('chrome://UserJS/content/io/_FILE_BASE.js');
...
function File (url) {
	if (!(this instanceof File)) {
		...
		this.fileInst = Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsILocalFile);
		this.fileInst.initWithPath(path);
	}
}

File.prototype = {
	get path () {
		return this.fileInst.path; 
	},
	exists: function () {
		return this.fileInst.exists();
	},
	get leafName () {
	   return this.fileInst.leafName;
	},
	set leafName (aLeaf) {
		this.fileInst.leafName = aLeaf;
	},
	get parent () {
		var parent = this.fileInst.parent;
	...
	get permissions () {	
		if (!this.exists()) {
			throw 'create the path first';
		}
		return parseInt(this.fileInst.permissions.toString(8)); 
	},
	set permissions (aPermission) {
		if (!this.exists()) {
			throw 'create the path first';
		}
		this.fileInst.permissions = aPermission; 
	},
	get lastModifiedTime () {
		if (!this.exists()) {
			throw 'create the path first';
		}
		return this.mFileInst.lastModifiedTime;
	},
	clone: function () {
		return new File(this.fileInst.path);
	},
	moveTo: function () {
		let argAry =  Array.prototype.slice.call(arguments, 0);
		this.mFileInst.moveTo.apply(this.mFileInst, argAry);
		return this;
	}
...
if (typeof(JS_LIB_LOADED)=='boolean') 
{
  ...
  */
  ...
  exists : function ()
  {
    if (!this.checkInst()) jslibErrorMsg("NS_ERROR_NOT_INITIALIZED");
