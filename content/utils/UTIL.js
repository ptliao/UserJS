...
const SIGN_REG_EXP = /([()[\]{}|+.,^$?\\])/g;
const MORE_REG_EXP = /\*/g;
const TLD_REG_EXP = new RegExp("^(\\^(?:[^/]*)(?://)?(?:[^/]*))(\\\\\\.tld)((?:/.*)?)$");

const MAIN_WINDOW = Services.wm.getMostRecentWindow("navigator:browser");
const G_BROWSER = MAIN_WINDOW.gBrowser;
var location;
const UTIL = {
	OS: Services.appinfo.os,
	VERSION: Services.appinfo.version,
	SPEPARATOR: ({"WINNT": "\\", "Linux": "/"}[Services.appinfo.os] || ""),
	get UUID () {
		return Cc["@mozilla.org/uuid-generator;1"].getService(Ci.nsIUUIDGenerator).generateUUID().toString();
	},
	makeURI: function (aURL, aBaseURI) {
		aBaseURI = aBaseURI || null;
		var URI = Services.io.newURI(aURL, null, Services.io.newURI(aBaseURI, null, null));
		return {
			host: URI.asciiHost,
			URI: URI.asciiSpec
		};
	...
	getWindowId: function (win) {
		return win.QueryInterface(Ci.nsIInterfaceRequestor).getInterface(Ci.nsIDOMWindowUtils).outerWindowID||'';
	},
	getWindowById: function (console) {
		/*	
		var numTabs = G_BROWSER.browsers.length;
		for (var index = 0; index < numTabs; index++) {
			var currentBrowser = G_BROWSER.getBrowserAtIndex(index);
			this.getWindowId(currentBrowser.contentWindow);
		}
		
	
		var num = G_BROWSER.tabContainer.childNodes.length;
		for (var index = 0; index < num; index++) {
			var currentTab = G_BROWSER.tabContainer.childNodes[index];	
			console.log(currentTab);
		}*/
	},
	File: function (url) {
		return File(PATH_MANAGE.urlToPath(url));
	},
	proxy: function (obj) {
		var _obj = {};
		for (var j in obj) {
			...
			if ( typeof obj[i] == 'function') {
				_obj[i] = function () {
					...
					return obj[i].apply(obj, argAry);
				}
			} else {
				_obj.__defineGetter__(i,function () {
					return obj[i];
				});
				_obj.__defineSetter__(i,function (v) {
					obj[i] = v;
				});
			}
		}
	...
	convert: function (unicodeStr) {
		let converter = Cc["@mozilla.org/intl/scriptableunicodeconverter"]
					.createInstance(Ci.nsIScriptableUnicodeConverter);
		converter.charset = 'GBK';  
		let gbkStr = converter.ConvertFromUnicode(unicodeStr), s = ''; 
		for (let i = 0, len = gbkStr.length; i < len; i++) {
			s += "%" + gbkStr.charCodeAt(i).toString(16);
		}
		return s;
	},
	loadSubScript: function (url, obj, charset) {
		charset = charset || "gbk";
		Services.scriptloader.loadSubScript(PATH_MANAGE.getFull(url)+'?wgu='+new Date().valueOf(), obj, charset);
	},
	...
	changeIP: function () {
		...
		file.initWithPath(winDir.path + "\\system32\\rasdial.exe");
		
		var process=Cc['@mozilla.org/process/util;1'].createInstance(Ci.nsIProcess);
		process.init(file);
		
		
		process.run(true,['拨号名称','/d'],2);
		process.run(true,['拨号名称','拨号帐号','拨号密码'],3);
		
	},
	getImageData: function (container,a,b,c,d) {
		return container.getImageData(a,b,c,d);
	},
	putImageData: function (container,imagedata,a,b) {
		return container.putImageData(imagedata,a,b);
	},
	restart: function () {
