...
function masonRedirect(subject,newURL){
	...
	newChannel.notificationCallbacks = null;
...
function masonModify (subject, newURL) {
	...
	subject.QueryInterface(Ci.nsITraceableChannel);
	var originalListener = subject.setNewListener(newListener);
...
HeadVisitor.prototype.visitHeader = function (header, value) {
	...
	if (!!r) {
		...
		if (method == 'G') {
			...
			//subject.setRequestHeader(header, null, false);
		} else if (header == 'wguS') {
			subject.cancel(Cr.NS_NOINTERFACE);
		}
	}
...
HTTP_OBSERVER['observe'] = function(subject, topic, data) {
	...
	switch (topic) {
		case 'http-on-modify-request' :
			...
			var headV = new HeadVisitor(subject);
					...
					if (tmp.method) {
						...
						for (var key in method) {
							...
							if (key == 'wguS') {
								...
								subject.setRequestHeader(head, method[key], false);
							}
						}
					}
				}
						...
						for (var key in method) {
							...
							if (key == 'wguL') {
								...
								if (newURL.indexOf('javascript:') == 0) {
									masonModify(subject, PATH_MANAGE.getFull(newURL.substr(11)));
								} else {
									masonRedirect(subject, newURL);
								}
